...
import { Plus, Trash2, Clock, Ban, CalendarOff, Palmtree, Loader2 } from "lucide-react"
...
export default function DisponibilidadPage() {
  const { availability, loading: availabilityLoading, updateAvailability } = useAvailability()
  ...
  const handleTimeChange = async (id: string, field: "start_time" | "end_time", value: string) => {
    ...
    if (blockType === "vacation_range") {
      ...
      for (const day of vacationDays) {
        ...
        const existing = overrides.find(
          (o) => o.date === dateStr && o.is_blocked && o.start_time === null && (!o.blocked_slots || o.blocked_slots.length === 0)
        )
        ...
        await addOverride({
          ...
          end_time: null,
          is_blocked: true,
          reason: overrideReason || `Vacaciones (${format(parseISO(vacationStart), "d MMM", { locale: es })} - ${format(parseISO(vacationEnd), "d MMM", { locale: es })})`,
          blocked_slots: [],
        })
      }
  ...
  const groupedOverrides = useMemo(() => {
    ...
    const groups: { type: "single" | "vacation"; ids: string[]; dates: string[]; reason: string | null; blockedSlots: string[] }[] = []
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      for (let j = i + 1; j < sorted.length; j++) {
        ...
        const isNextFullDay = !next.blocked_slots || next.blocked_slots.length === 0
  ...
  return (
    <AdminLayout>
      <div className="space-y-8">
        {/* Weekly schedule */}
        <section>
          <h2 className="mb-4 font-serif text-lg text-foreground">Horario semanal</h2>
                ...
                {a.is_active ? (
                  <div className="flex items-center gap-2">
                    <input
                      type="time"
                      value={a.start_time}
                      onChange={(e) => handleTimeChange(a.id, "start_time", e.target.value)}
                      className="rounded-md border bg-card px-2 py-1.5 font-mono text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                    />
                    <span className="text-sm text-muted-foreground">a</span>
                    <input
                      type="time"
                      value={a.end_time}
                      onChange={(e) => handleTimeChange(a.id, "end_time", e.target.value)}
                      className="rounded-md border bg-card px-2 py-1.5 font-mono text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                    />
                  </div>
                ) : (
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-serif text-lg text-foreground">Excepciones y bloqueos</h2>
            <Button
              size="sm"
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium capitalize text-foreground">
                          {format(parseISO(group.dates[0]), "EEEE d 'de' MMMM yyyy", { locale: es })}
                        </p>
                        {group.reason && (
                          <p className="text-xs text-muted-foreground">{group.reason}</p>
                        )}
                        {isFullDay ? (
                          <p className="mt-0.5 text-xs font-medium text-destructive">Dia completo bloqueado</p>
                        ) : (
      ...
      {/* Add override dialog */}
      <Dialog open={showAddOverride} onOpenChange={(open) => { if (!open) resetDialog() }}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Agregar bloqueo</DialogTitle>
            <DialogDescription>
              Bloquea un dia completo, turnos especificos o un rango de dias para vacaciones.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="mb-2 block text-sm font-medium text-foreground">Tipo de bloqueo</label>
              <div className="flex gap-2">
                <button
                  onClick={() => { setBlockType("full_day"); setSelectedSlots([]); setVacationStart(""); setVacationEnd("") }}
                  className={cn(
                ...
                <button
                  onClick={() => { setBlockType("specific_slots"); setVacationStart(""); setVacationEnd("") }}
                  className={cn(
                    "flex flex-1 flex-col items-center justify-center gap-1 rounded-lg border px-3 py-2.5 text-xs font-medium transition-colors",
                ...
                <button
                  onClick={() => { setBlockType("vacation_range"); setOverrideDate(""); setSelectedSlots([]) }}
                  className={cn(
                    "flex flex-1 flex-col items-center justify-center gap-1 rounded-lg border px-3 py-2.5 text-xs font-medium transition-colors",
                ...
                {vacationDayCount > 0 && (
                  <div className="col-span-2 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <Palmtree className="h-4 w-4 text-primary" />
                      <p className="text-sm font-medium text-foreground">
                        {vacationDayCount} dia{vacationDayCount > 1 ? "s" : ""} seleccionado{vacationDayCount > 1 ? "s" : ""}
                      </p>
                    </div>
                    <p className="mt-1 text-xs text-muted-foreground">
                      {format(parseISO(vacationStart), "EEEE d 'de' MMMM", { locale: es })} {" - "}
                      {format(parseISO(vacationEnd), "EEEE d 'de' MMMM yyyy", { locale: es })}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder={blockType === "vacation_range" ? "Ej: Vacaciones de verano" : "Ej: Feriado, Turno personal"}
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                    {slotsForDate.map((slot) => {
                      ...
                      return (
                        ...
                        >
                          {isSelected && <Ban className="mb-0.5 mr-1 inline h-3 w-3" />}
                          {slot}
                        </button>
                      )
                    })}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                ...
                (blockType === "specific_slots" && (!overrideDate || selectedSlots.length === 0)) ||
                (blockType === "vacation_range" && (!vacationStart || !vacationEnd || vacationDayCount === 0))
              }
            >
              {blockType === "vacation_range" ? "Bloquear vacaciones" : "Agregar bloqueo"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
