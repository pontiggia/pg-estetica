...
export async function GET(request: NextRequest) {
  ...
  const fullDayBlock = overrides?.find(
    (o) => o.is_blocked && !o.start_time && !o.end_time && (!o.blocked_slots || o.blocked_slots.length === 0)
  )
  if (fullDayBlock) {
    return NextResponse.json({ slots: [], available: false })
  }
  ...
  const takenSlots = new Set(appointments?.map((a) => a.start_time) ?? [])
  ...
  while (current + 60 <= endMinutes) {
    ...
    const isBlocked = overrides?.some((o) => {
      ...
      if (o.blocked_slots && o.blocked_slots.length > 0) {
        return o.blocked_slots.includes(timeStr)
      }
